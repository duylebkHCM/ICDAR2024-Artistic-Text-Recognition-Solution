...
def main():
    ...
    hp = model.hparams
    datamodule = SceneTextDataModule(args.data_root, hp.img_size, hp.max_label_length, hp.charset_train,
                                     hp.charset_test, args.batch_size, args.num_workers, False, rotation=args.rotation)

    results = {}
    for name, dataloader in datamodule.test_dataloaders(args.data_root).items():
        total = 0
        correct = 0
        ned = 0
        confidence = 0
        label_length = 0
        for imgs, labels in tqdm(iter(dataloader), desc=f'{name}'):
