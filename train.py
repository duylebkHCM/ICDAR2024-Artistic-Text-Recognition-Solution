...
from strhub.custom_trainer import CustomTrainer
...
@hydra.main(config_path='configs/wordart', config_name='main', version_base='1.2')
def main(config: DictConfig):
    trainer_strategy = None
    # json_liked = OmegaConf.to_yaml(config)
    # print(json_liked)
    ...
    with open_dict(config):
        ...
        if isinstance(config.data.root_dir, ListConfig):
            root_dir=OmegaConf.to_object(config.data.root_dir)
            root_dirs = []
            for r in root_dir:
                r=hydra.utils.to_absolute_path(r)
                root_dirs.append(r)
            config.data.root_dir = root_dirs
        else:
    ...
    if config.pretrained is not None:
        model.load_state_dict(get_pretrained_weights(config.pretrained, model=model))
    print(summarize(model, max_depth=1 if model.hparams.name.startswith('parseq') else 2))

    datamodule: SceneTextDataModule = hydra.utils.instantiate(config.data, tokenizer=model.tokenizer)
