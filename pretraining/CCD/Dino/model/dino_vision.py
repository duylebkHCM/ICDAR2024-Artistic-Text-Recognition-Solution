...
class ABIDINOModel(nn.Module):
    ...
    def forward(self, x, metrics, target_mask, epoch, clusters=None, index=None):
        ...
        backbone_out, Segmentation_input, backbone_out_size = self.backbone(torch.cat([x1, x2]))
        
        N, T, E = backbone_out.shape
        region_f = backbone_out.reshape(N, backbone_out_size[0], backbone_out_size[1], E).permute(0, 3, 1, 2)
        
        if clusters is None: #student branch
            Segmentation_out = self.segmentation(Segmentation_input)
            
            if epoch < 30:
                B, H, W = target_mask.shape # H=32, W=128
