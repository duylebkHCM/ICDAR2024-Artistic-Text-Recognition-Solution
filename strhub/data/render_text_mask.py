...
if __name__ == '__main__':
    ...
    # different chars have different pixel value
    voc = list(string.printable[:-6])
    class_to_idx = dict(zip(voc, range(len(voc))))
    ...
    chars = 'abcedf'
    surf, bbs = render_curved(font, chars, 0.5)
    cv2.imwrite('binary_curve.png', cv2.resize(surf, (128, 32)))
    # modify the value of each chars to its category
    cate_aware_surf = np.zeros((surf.shape[0], surf.shape[1], len(class_to_idx))).astype(np.uint8)
    for char, bb in zip(chars, bbs):
        char_id = class_to_idx[char]
        cate_aware_surf[:, :, char_id][bb[1]:bb[1]+bb[3], bb[0]:bb[0]+bb[2]] = surf[bb[1]:bb[1]+bb[3], bb[0]:bb[0]+bb[2]]

    cate_aware_surf[cate_aware_surf>0.5] = 1
    cate_aware_surf[cate_aware_surf<=0.5] = 0
    fg_x = cate_aware_surf.sum(-1) # [H, W]
    cate_aware_surf = cate_aware_surf.argmax(-1) # [H, W]
    cate_aware_surf[fg_x==0] = len(class_to_idx) # background
    cate_aware_surf = cate_aware_surf.reshape((surf.shape[0], surf.shape[1]))
    ## category shift
    cate_aware_surf = cate_aware_surf + 1
    cate_aware_surf[fg_x==0] = 0
    cate_aware_surf = cate_aware_surf/cate_aware_surf.max() * 255
    cate_aware_surf = cate_aware_surf.astype(np.uint8)
    cate_aware_surf = cv2.applyColorMap(cate_aware_surf, cv2.COLORMAP_HOT)
    cv2.imwrite('class_curve.png', cate_aware_surf)
