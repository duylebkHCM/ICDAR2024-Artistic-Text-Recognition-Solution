...
def calc_channel_mean_std(feat):
    ...
    assert (
        W * H != 1
    ), f"Cannot calculate std over W, H {size} (N,C,H,W), dimensions W={W}, H={H} cannot be 1"
...
def calc_width_mean_std(feat):
    ...
    assert (
        W != 1
    ), f"Cannot calculate std over W {size} (N,C,H,W), dimensions W={W} cannot be 1"
...
def get_adain_dim(dim):
    ...
    assert (
        dim in dim2func
    ), f"Please insert one of the following : {list(dim2func.keys())}"
    return dim2func[dim]
...
class PermuteAdaptiveInstanceNorm2d(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f"p={self.p}, mode={self.mode_func.__name__}"
...
class TextAdaIN(PermuteAdaptiveInstanceNorm2d):
    def __init__(self, p=0.01, dim=("C", "H"), num_windows=5, **kwargs):
        ...
        :param p: the probability of applying Permuted AdaIN
        """
        super().__init__(p=p, dim=dim, **kwargs)
    ...
    def forward(self, x):
        ...
        x = torch.cat(
            (x_without_remainder, x[:, :, :, frame_total:]), dim=3
        ).contiguous()
        return x

    def extra_repr(self) -> str:
        return "p={}, num_windows={}  mode={}".format(
            self.p, self.num_windows, self.mode_func.__name__
        )
...
def revert_windowed_tensor(x_without_remainder, k, W):
    ...
    N = N // k
    frame_size = W // k
    x_without_remainder = x_without_remainder.transpose(
        1, 3
    )  # N*k x frame_size x H x C
    x_without_remainder = x_without_remainder.reshape(
        N, k * frame_size, H, C
    )  # revert the windows back to their original position
    x_without_remainder = x_without_remainder.transpose(
        1, 3
    )  # N x C x H x k*frame_size
...
def create_windows_from_tensor(x, k):
    ...
    x_without_remainder = x_without_remainder.reshape(
        N * k, frame_size, H, C
    )  # N*num_windows x frame_size x H x C
    x_without_remainder = x_without_remainder.transpose(
        1, 3
    ).contiguous()  # N*num_windows x C x H x frame_size
    return x_without_remainder
