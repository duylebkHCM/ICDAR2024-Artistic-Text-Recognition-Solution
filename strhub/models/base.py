...
class CrossEntropySystem(BaseSystem):
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str]) -> Tuple[Tensor, Tensor, int]:
        ...
        logits = self.forward(images, max_len) #do not set max_len base on current lenght of current batch in testing stage, because vocab in test and train may differ,
        # for example in train using vocab with 94 token, and in test use vocab with only 36 token, so the label in test stage may by differ based on which vocab used to preprocess
        # and therefore the max_len based on test vocab is less than train vocab and so the predicted will be truncated, and differ from the label
